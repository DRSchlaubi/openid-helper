...
fun RequestInterceptorBuilder.RequestContext<*>.validateClientSecret(clientId: String, clientSecret: String) {
    ...
    if (header is HttpAuthHeader.Single) {
        if (header.authScheme.equals("Basic", ignoreCase = true)) {
            if (header.blob != "$clientId:$clientSecret".encodeBase64())
                throw IllegalArgumentException("Invalid client credentials")
        }
    }
    if (data is Parameters) {
        if (clientId != data["client_id"]) throw IllegalArgumentException("Invalid client credentials")
        if (clientSecret != data["client_secret"]) throw IllegalArgumentException("Invalid client credentials")
    }
}